...
# Define the new initFiscal function
$newFunction = @'
...
# Replace the old function with the new one using regex
$pattern = '(?s)        async function initFiscal\(\) \{.*?            \} catch \(e\) \{ console\.error\("Fiscal link error", e\); \}\r\n        \}'
$content = $content -replace $pattern, $newFunction

# Write back to file
$content | Set-Content $filePath -NoNewline

Write-Host "✅ Successfully updated initFiscal function in Dashboard.cshtml"
